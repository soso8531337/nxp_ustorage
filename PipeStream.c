...
uint8_t Pipe_Streaming2(uint8_t corenum,uint8_t pipe, uint8_t* const buffer, uint32_t const buflen, uint32_t *actual){
	pipeselected[corenum] = pipe;
	printf("pipe=%d\r\n",pipe);
	uint8_t status;
	uint32_t pipehdl = PipeInfo[corenum][pipeselected[corenum]].PipeHandle;
	if (HCD_STATUS_OK == HcdGetPipeStatus(pipehdl)){
		HcdSetStreamPacketSize(pipehdl,512);
		HcdDataTransfer(pipehdl,buffer,buflen,&PipeInfo[corenum][pipeselected[corenum]].ByteTransfered);
		status = Pipe_WaitUntilOK(corenum);
		if(status != 0){
			printf("pipe error,status=%d\r\n",status);
			return status;
		}
		//printf("size = %d\r\n",PipeInfo[corenum][pipeselected[corenum]].ByteTransfered);
		if(actual){
			*actual = PipeInfo[corenum][pipeselected[corenum]].ByteTransfered;
		}
		return HCD_STATUS_OK;
		
	}else return PIPE_RWSTREAM_IncompleteTransfer;	
}
uint8_t Pipe_WaitUntilOK(uint8_t corenum)
{
	uint8_t status;
	while(1){
		status = HcdGetPipeStatus(PipeInfo[corenum][pipeselected[corenum]].PipeHandle);
		if(status == HCD_STATUS_OK){
			return 0;
		}else if(status == HCD_STATUS_TRANSFER_Stall){
			return HCD_STATUS_TRANSFER_Stall;
